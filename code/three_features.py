...
from predunder.training import train_random_forest, train_xgboost, train_kfold
...
def initialize_parser():
    ...
    parser.add_argument("-f", "--feature", help="feature to include aside from age and area type",
                        choices=['HDD', 'IDD', 'none'],
                        default='HDD'
                        )
    ...
    return config['task'], config['reset'], config['feature']


if __name__ == '__main__':
    TASK, to_reset, extra_feature = initialize_parser()
    ...
    FEATURES = ['AGE', 'AREA_TYPE']
    if extra_feature != 'none':
        if extra_feature == 'HDD':
            FEATURES.append("HDD_SCORE")
        elif extra_feature == 'IDD':
            FEATURES.append("IDD_SCORE")

    train_df = train_df[FEATURES+[TASK]]
    test_df = test_df[FEATURES+[TASK]]
    RESULTS_DIR = os.path.join(RESULTS_DIR, f'three-features-{extra_feature}')
    LATEX_DIR = os.path.join(LATEX_DIR, f'three-features-{extra_feature}')
